...
pub struct RawAVec<T, A: Alloc> {
    pub(crate) ptr: NonNull<T>,
    pub(crate) cap: usize,
    pub(crate) alloc: A
}

impl<T, A: Alloc> RawAVec<T, A> {
    /// Make a new array.
    #[inline]
    pub const fn new_in(a: A) -> Self { RawAVec { ptr: NonNull::dangling(), cap: 0, alloc: a } }
    ...
    pub fn with_capacity_in(mut a: A, cap: usize) -> Option<Self> {
        if mem::size_of::<T>() == 0 {
            Some(RawAVec { ptr: NonNull::dangling(), cap, alloc: a })
        } else if cap == 0 {
            Some(RawAVec::new_in(a))
        } else {
            a.alloc_array(cap).ok().map(|ptr| RawAVec { ptr, cap, alloc: a })
        }
    }

    /// Return number of elements array can hold before reallocation.
    #[inline] pub(crate) fn capacity(&self) -> usize { self.cap }

    #[inline] pub(crate) unsafe fn storage_mut(&mut self) -> &mut [T] {
        slice::from_raw_parts_mut(self.ptr.as_ptr(), self.cap)
    }

    #[inline] pub(crate) unsafe fn storage(&self) -> &[T] {
        slice::from_raw_parts(self.ptr.as_ptr(), self.cap)
    }

    #[inline] pub(crate) fn ptr(&self) -> *mut T {
        self.ptr.as_ptr() as *const T as *mut T
    }
    ...
    pub unsafe fn from_raw_parts_in(alloc: A, ptr: *mut T, cap: usize) -> Self {
        RawAVec { ptr: NonNull::new_unchecked(ptr), cap, alloc }
    }
...
impl<T, A: Alloc + Default> RawAVec<T, A> {
    /// Make a new array.
    #[inline]
...
impl<T, A: Alloc> Drop for RawAVec<T, A> {
    #[inline]
    fn drop(&mut self) {
...
impl<T, A: Alloc + Default> Default for RawAVec<T, A> {
    #[inline]
    fn default() -> Self { RawAVec::new() }
}

#[inline]
unsafe fn alloc_or_realloc<T, A: Alloc>(
    a: &mut A,
    ptr: NonNull<T>,
    m: usize,
    n: usize
) -> Result<(NonNull<T>, usize), AllocErr> {
    (if 0 == m { a.alloc_array(n) } else { a.realloc_array(ptr, m, n) }).map(|ptr| (ptr, n))
}
